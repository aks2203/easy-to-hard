""" recurrent_net.py
    Recurrent chess playing neural network.
    May 2021
"""
...
class RecurChessNet(nn.Module):
    ...
    def forward(self, x):
        if self.training:
            ...
            self.thoughts = torch.zeros(self.iters, x.size(0), 2, 8, 8).to(x.device)
            out = F.relu(self.conv1(x))
            for i in range(self.iters):
                out = self.recur_block(out)
                thought = F.relu(self.conv2(out))
...
def recur_net(depth, width, **kwargs):
    return RecurChessNet(BasicBlock, [2], width, depth)
