...
def main():
    ...
    if args.lr_decay.lower() == "step":
        ...
        lr_scheduler = CosineAnnealingLR(optimizer, args.epochs, eta_min=0,
                                         last_epoch=start_epoch-1,
                                         verbose=False)
    else:
    ...
    if args.test_iterations is not None:
        assert isinstance(net.module.iters, int), f"{ic.format()} Cannot " \
                                                  f"test feed-forward model with iterations."
    ...
    if not args.hist:
        ...
        hist_tensor, eval_acc = test(net, evalloader, "hist", device)
        torch.save(hist_tensor, f"{args.output}/{train_log[:-4]}"+".pth")
